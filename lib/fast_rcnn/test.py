...
def im_detect(net, im, _t, boxes=None):
	...
	#print "bbox_pred_name:",bbox_pred_name
...
def im_detect_hierarchy(net, im, _t, boxes=None):
    """Detect object classes in an image given object proposals.
    ...
    if cfg.TEST.SVM:
        ...
        # were trained as linear SVMs
        scores = net.blobs['cls_score'].data
        sub_scores = net.blobs['sub_cls_score'].data
    else:
        # use softmax estimated probabilities
        scores = blobs_out['cls_prob']
        sub_scores = blobs_out['sub_cls_prob']
    ...
    return scores, sub_scores, pred_boxes
